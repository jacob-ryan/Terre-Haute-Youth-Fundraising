                        ...
                        for (var j = 0; j < eventsOcc.length; j++) {
                            if (registered[i].eventOccurrenceId == eventsOcc[j].id) {
                                ...
                                if (registered[i].isPaid == false) {
                                    var activeButton = "<a href = #/event-payment/" + eventsOcc[j].type + "/" + registered[i].id + "><button>Pay Now</button></a>";
                                    userData[count].push(eventsOcc[j].type, eventsOcc[j].date, eventsOcc[j].description, true, activeButton);
                                } else {
                                    userData[count].push(eventsOcc[j].type, eventsOcc[j].date, eventsOcc[j].description, false, deActButton);
                                }
                                count++;
                            }
                        }
